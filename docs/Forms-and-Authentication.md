...
Primero agregaremos las funciones para login y para retornar la vista de login en `SessionsController`. Debemos usar `session()->regenerate()` para evitar posibles session fixation attacks.
...
Agregamos las rutas necesarias para mostrar la vista de login y para hacer el login
...
y para terminar, creamos `/resources/views/sessions/create.blade.php` la cual sera la vista desde donde haremos login.
